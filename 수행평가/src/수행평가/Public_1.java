	...
	public void public_1(){
		...
		String sc_go_3lang[] = {
			...
			Scanner sc = new Scanner(System.in);
				...
				System.out.println(tema[2]);		//공공장소에서의 내용.
				System.out.println("〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓");
				System.out.println("한번더 선택해 주세요");
				System.out.println("〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓");
				int User_B = sc.nextInt();					// #패스트푸드점,쇼핑할때,대중교통 에서를 선택해하도록
				System.out.print(tema_go[User_B]);			//그 선택한 내용들 뽑아옴. 
				int User_C = sc.nextInt();						//거기서 한번 더 선택.

				if (User_B == 0) {							//패스트푸드를 했으면 tema_go에서 [0]의 내용을 뽑아오고 
					System.out.println(sc_go_1lang[User_C]);			//패스트푸드에서 나왔던 문장들을 선택하게한 후(User_C) , sc_go_1lang을 뽑아온다.
				} else if (User_B == 1) {
