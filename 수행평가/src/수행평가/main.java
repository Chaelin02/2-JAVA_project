	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (User_A == 0) { // 만약 USer_A가 0을 눌렀을 때
				...
				public_1.public_1();
			} else if (User_A == 3) { // User_A가 3을 눌렀을 때
				Life life = new Life(); // 일상생활에서의 객체 생성
				life.life();
			}
			// 위의 while문 안에 User_A가 선택한 객체들이 다 실행하고 나서
			// 종료를 할 것인지 아니면 계속 찾아 볼 것인지 물어봄. 여기서부터
			while (true) {
				System.out.println("표현보기를 종료하시겠습니까?(y/n) : ");
				String answer = sc.next(); // n/y를 입력받음 .
